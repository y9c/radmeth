...
static size_t parse_natural_number(string encoding) {
...
static void parse_row_name(string row_name_encoding, TableRow& row) {
  ...
  if (num_colon != 3)
    throw SMITHLABException(
        ...
        "a line chromosome:start:end:strand. Got \"" +
  ...
  if (row.chrom.empty())
    throw SMITHLABException(
        "Error parsing " + row_name_encoding + ": chromosome name is missing.");
...
static void parse_row_read_counts(
    istringstream& row_name_encoding,
    TableRow& row) {
  ...
  if (!row_name_encoding.eof())
    throw SMITHLABException(
        "some row entries are not natural numbers: " + row_name_encoding.str());
}

void read_row(std::string row_encoding, TableRow& row) {
...
bool has_low_coverage(
  ...
  for (size_t sample = 0; sample < design.num_samples(); ++sample) {
    if (design(sample, test_factor) == 1) {
      ...
      if (row.total_counts[sample] != 0) is_covered_in_other_samples = true;
    }
  }
...
bool has_extreme_counts(const Design& design, const TableRow& row) {
  ...
  for (size_t sample = 0; sample < design.num_samples(); ++sample) {
    ...
    if (row.meth_counts[sample] != 0) is_unmethylated = false;
  }
